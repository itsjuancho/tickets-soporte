...
isLogin();
redireccionar("/ticketsTienda/acp/soporte/cerrar.php","cerrar_ticket");
	...
	if (!empty($_POST['tid']) && !empty($_POST['t_id'])) {
		...
		if (is_numeric($t_id)) {
			...
			if (mysqli_num_rows($t) > 0) {
				...
				if ($tid == md5(sha1(md5(sha1(md5($p['unique_key'])))))) {
					$sqlUpdateEstado = "UPDATE ticket SET estadoTicket = 2 WHERE idTicket = $t_id";
					$exeUpEt = mysqli_query($conexion,$sqlUpdateEstado);
					if ($exeUpEt) {
						$_SESSION['msgReplyTicket'] = "<i class='far fa-check-circle'></i> Has cerrado el ticket!";
