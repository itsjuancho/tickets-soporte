...
isLogin();
redireccionar("/ticketsTienda/acp/soporte/abrir.php","abrir_ticket");
if (isset($_POST['tid']) && isset($_POST['t_id'])) {
	if (!empty($_POST['tid']) && !empty($_POST['t_id'])) {
		...
		if (is_numeric($t_id)) {
			$sql = "SELECT unique_key,user_id,id_admin FROM ticket WHERE idTicket = $t_id";
			...
			if (mysqli_num_rows($t) > 0) {
				...
				if ($tid == sha1(md5(sha1(md5($p['unique_key']))))) {
					if ($p['id_admin'] == NULL) {
						$sqlUpdateEstado = "UPDATE ticket SET estadoTicket = 0 WHERE idTicket = $t_id";
					}else{
						$sqlUpdateEstado = "UPDATE ticket SET estadoTicket = 1 WHERE idTicket = $t_id";
					}
					$exeUpEt = mysqli_query($conexion,$sqlUpdateEstado);
					if ($exeUpEt) {
						$_SESSION['msgReplyTicket'] = "<i class='far fa-check-circle'></i> Has abierto el ticket nuevamente.";
						$_SESSION['typeMsgReply'] = "success";
						header('location: ticket/'.$t_id);
						exit();
					}else{
						$_SESSION['msgReplyTicket'] = "No se pudo abrir el ticket. Contáctanos a contacto@juanchoo.com adjuntando el numero del ticket para revisar el error.";
						$_SESSION['typeMsgReply'] = "danger";
						header('location: ticket/'.$t_id);
						exit();
					}
